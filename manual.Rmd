...
Another important property of copulas is that their component variables are invariant to monotonic (strictly increasing) transformations. This property allows one to transform the components from Uniform(0,1) into any other closed form distribution via the inversion of the semi-parametric CDF [@rupeprt; @ruenzi2011]. MCmarket provides users with the functionality to transform the marginal distributions into normal, student-t or skewed generalized t distributions. it is important to note that this step is deterministic and therefore involves no randomness given the realisations from the random uniformly distributed draws.  
