...
## Aim
The aim of this project is to develop a general and easy to use Monte Carlo package that generates asset return data with a prespecified correlation matrix adn various other risk factors.These factors include a leverage parameter that determines the likelihood of joint drawdowns, as well as the parameters of a ARIMA(p,q) + GARCH(q,p) process. Elliptical copulas are used to induce the correlation in the simulated data, while Archmedian copulas are used to adjust the likelihood of joint drawdowns. 
...
Corr <- gcVar(N = 50, Clusters = "overlapping", Num_Layers = 3, Num_Clusters = c(2,5,10))
Corr %>% corrplot::corrplot()
