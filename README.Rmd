...
Since it would also be useful for MCmarkets's users to have easy acess to some labeled empirical correlation matrices, a data set containing correlation matricies labeled "stressed", "rally" and "normal" is now constructed.
...
Tawn's (1988) Theorem: States that a copula is a convex set and every convex combination of existing copula functions is again a copula. 
...
```{r sim_inno function, eval=FALSE, include=TRUE}
#' @title sim_market
...
This is used if user wants to see a progress bar when performing MC simulations.

```{r sim_market_with_progress, eval=FALSE, include=TRUE}
...
```{r sim_market_with_progress example}
N <- 10
pb <- dplyr::progress_estimated(N)   # this must be named pb
market <-
       map_dfr(1:N,
